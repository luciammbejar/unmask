      ...
      try{
        ...
        if(state.ai){
          const {data:aiPlayer,error:aiError}=await db.from('players').insert({
            game_id:game.id,
            name:['Alex','Sam','Dani','Noa','Cris'][Math.floor(Math.random()*5)],
            player_number:null,
            color:colors[Math.floor(Math.random()*colors.length)],
            is_host:false,
            is_ai:true
    ...
    async function loadAll(redraw=false){
      ...
      // Comprobamos el estado de la partida en cada polling.
      // Así el móvil entra aunque Realtime no entregue el evento.
      ...
      // Si la pregunta todavía no existe, el anfitrión la reintenta.
    ...
    async function generateQuestion(){
      ...
      try{
        ...
        // La pregunta se muestra en su panel; todo lo que escriben
        // los jugadores pasa por el único chat libre.
    ...
    async function sendMsg(){
      ...
      // Solo el anfitrión programa a la IA para evitar duplicados.
